...
function isPalindrome(str) {
  ...
  // let strJoined = strLower.split(' ').join('');

  // if (
  //   strJoined[strJoined.length - 1] === '!' ||
  //   strJoined[strJoined.length - 1] === '?'
  // ) {
  //   strJoined = strJoined.slice(0, strJoined.length - 2);
  // }

  const strJoined = strLower.replace(/[.,!?\s]/g, '');

  for (let i = 0; i < strJoined.length; i += 1) {
    if (strJoined[i] !== strJoined[strJoined.length - 1 - i]) {
      return false;
    }
  }
